...
void versiiAnalysisRemoveIndivFrames(TString filename, int off = 1, int rtf = 0){ // 1=incl off data, 0=no off data, int rtf for remove tracking frames, 0 = no, 1 = yes
  ...
  for (int ip=0; ip<keyList->GetSize(); ip++){
	  ...
	  cfNorm = subtractOne(cfNorm); // subtract one so cf is centered around 0, and forced normalization
