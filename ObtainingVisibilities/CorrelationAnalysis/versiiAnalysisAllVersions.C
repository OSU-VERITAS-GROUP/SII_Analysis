// josie 13 nov 2023, 10 feb 2024
// new full analysis script, processes as many telescope pairs as are in the file from versii, takes root file versions -3 or -4
...
// run with: root versiiAnalysisAllVersions.C\(\"eta\ Tau_y2023m12d28h23m54s52_ZippedFrames.root\",1\)
...
TString GetTelescopeIdentifier(string FileName){ // function copied from multithread freq domain - to get indiv telescopes from filenames
  std::size_t found = FileName.find_last_of("/");
  string id = FileName.substr(found+1,2);
  return id;
}

void versiiAnalysisAllVersions(TString filename, int off = 1){ // off 1 = YES off data, 0 = NO off data
  ...
  TString runpy;
  TString py = "python3 CalcMvtUVonly.py";
  TString length = "0";
  TString loctime = "0";
  TString nframes = "0";
  TString frameSize = "0";
  ...
  for (int ip=0; ip<keyList->GetSize(); ip++){
      ...
      if ((keyName.Data()[0]=='T')&&(className=="TDirectoryFile")){ // if we find directory that start w "T" - take it as a pair
          ...
          if (off == 1){
              ...
              dOffadc1 = (afterADC1 - beforeADC1)/double(cfOrig->GetYaxis()->GetNbins());
              dOffadc2 = (afterADC2 - beforeADC2)/double(cfOrig->GetYaxis()->GetNbins());
    ...
    // ADD remove one if not doing noise removal
	  ...
	  if((t1==1 && t2==2)){reltimePar = -5;} // need to adjust these??
	  if((t1==1 && t2==3)){reltimePar = -10;} 
	  if((t1==1 && t2==4)){reltimePar =  2;}  
	  if((t1==2 && t2==3)){reltimePar =  10;} 
	  if((t1==2 && t2==4)){reltimePar =  18;} 
	  if((t1==3 && t2==4)){reltimePar =  8;} 
