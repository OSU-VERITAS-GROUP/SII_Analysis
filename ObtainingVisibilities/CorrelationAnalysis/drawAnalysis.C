...
void drawAnalysis(TString filename){//, int off = 1){ // bool doesn't work - let's try an int
int off = 1; // idk man
  ...
  // print output plots to pdf
  TCanvas* c1 = new TCanvas("","",1400,800);
  c1->Divide(3,1);
  TString pdfName = filename.ReplaceAll(".root",".pdf");
  cout << "saving some plots to output pdf named: plots_" << pdfName.Data() << endl;
  c1->Print(Form("plots_%s[",pdfName.Data()));
  ...
  for (int ip=0; ip<keyList->GetSize(); ip++){
      ...
      if ((keyName.Data()[0]=='T')&&(className=="TDirectoryFile")){ // if we find directory that start w "T" - take it as a pair
          ...
          c1->cd(3); gPad->cd(1); geom->Draw("baseline:time");
          c1->cd(3); gPad->cd(2); geom->Draw("OPD:time");
          c1->cd(3); gPad->cd(3); geom->Draw("v:u");
    ...
    c1->cd(1); gPad->cd(5);
	  ...
	  // draw the heatmap
	  cfNorm->SetTitle(Form("heatmap T%sT%s;relative time (ns);time in run (s)", tee1.Data(), tee2.Data()));
	  ...
	  c1->cd(2); gPad->cd(1); cfNorm->Draw("COLZ");
	  c1->cd(2); gPad->cd(2); cfFlat->Draw();
	  c1->Print(Form("plots_%s",pdfName.Data()));
...
double DoFFT (TProfile2D * CF, int times){
 ...
 //double nyquistFrequency_Hertz        = nyquistFrequency_RadianPerSec / (2.0*TMath::Pi());  //to make hertz
  ...
  //c1->cd(1); if(times == 1){ gPad->cd(5); }  else if(times == 2){ gPad->cd(6); }
  //FFTCombine->Draw();
  // ok I can either pass the canvas to the function or could make the function return the TH1D and draw like that
