...
while observing_time < end_observing_time:
    ...
    for iwhich in range(0,6):
        ...
        w[iwhich] = np.append(w[iwhich], (Dvec[iwhich,0]*-1*math.sin(hrang)*math.cos(decl)) + (Dvec[iwhich,1]*-1*math.sin(lat)*math.cos(hrang)*math.cos(decl) + Dvec[iwhich,1]*math.cos(lat)*math.sin(decl)) + (Dvec[iwhich,2]*math.cos(lat)*math.cos(hrang)*math.cos(decl) + Dvec[iwhich,2]*math.sin(lat)*math.sin(decl)))

        #print("bl", baselinemag[iwhich], "uv mag", math.sqrt((u[iwhich]*u[iwhich]) + (v[iwhich]*v[iwhich]))) # checked and they are nearly identical (but w opd and w opp signs)
        #print("opd", delay[iwhich], "w", w[iwhich]/3e8) 
...
for iwhich in range(0,6):
    ...
    for ipt in range(0, times.size): 
        outfile.write('%d   %f   %f   %f   %f   %f\n' % (frames[ipt], times[ipt], u[iwhich][ipt], v[iwhich][ipt], baselinemag[iwhich][ipt], 1.0e9*delay[iwhich][ipt]))
    outfile.close()
